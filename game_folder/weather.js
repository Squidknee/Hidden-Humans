class weather{
 ...
 loadBackground(){
   ...
   this.getWeather(this.getLocation.pos).then(weatherCode => {
      this.chooseBackground(weatherCode)
      console.log(weatherCode);
   })
 });
 
  }
 chooseBackground(weatherCode) {
 
   if(this.arr1.includes(weatherCode) ) {
     this.gameBackground = "grass"
   }else if(this.arr2.includes(weatherCode)) {
 ...
 // testbutton.addEventListener('click', loadBackground());
 getBackground() {
 return this.gameBackground;
 }
